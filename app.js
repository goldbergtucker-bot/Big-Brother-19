 ...
 * V7 adds:
 *   - official BB19 competition names/descriptions
 *   - editable starting relationships
 *   - editable custom alliances
 *   - separate Teams and Alliances
 */
(() => {
  const STORAGE_KEY = "bb19CustomSimulatorBrantsteeleV16";
  const LEGACY_STORAGE_KEYS = ["bb19CustomSimulatorBrantsteeleV14","bb19CustomSimulatorBrantsteeleV13","bb19CustomSimulatorBrantsteeleV12","bb19CustomSimulatorBrantsteeleV11","bb19CustomSimulatorBrantsteeleV10","bb19CustomSimulatorBrantsteeleV9","bb19CustomSimulatorBrantsteeleV8","bb19CustomSimulatorBrantsteeleV7"];
  const REVEAL_KEY = "bb19CustomSimulatorBrantsteeleV16Index";
  let state = GameState.createInitialState(BB19_CONFIG);
  let history = [];
  let pointer = -1;
  let activeTab = "event";
  ...
  const demoNames = [["Tucker","Player"],["Grace","Player"],["Antonio","Player"],["Riley","Player"],["Aly","Player"],["Stephanie","Player"],["Jordan","Player"],["Morgan","Player"],["Cameron","Player"],["Taylor","Player"],["Alex","Player"],["Casey","Player"],["Drew","Player"],["Jamie","Player"],["Logan","Player"],["Parker","Player"],["Quinn","Player"]];
  ...
  function competitionCard(entry) {
    // Always resolve the official competition directly from the BB19 schedule
    ...
    const official = BB19_CONFIG.competitionSchedule?.find(x => Number(x.week) === Number(entry.week) && x.type === entry.type);
    ...
    return `<section class="competition-card">
      <div class="competition-top"><span class="competition-kicker">${esc(type)}</span><span class="official-badge">REAL BB19 COMPETITION</span></div>
      <h3>${esc(nameValue)}</h3>
  ...
  function weeklySummaryRows(){
    ...
    povIndexes.forEach(({i})=>{
      ...
      const wildcard=findLast("temptation-competition")||findLast("den-of-temptation")||null;
  ...
  function renderWeeklySummary(){
    ...
    const cards=rows.map(r=>`<article class="weekly-summary-card"><h3>${esc(r.label)}</h3><div class="weekly-summary-rows"><div><strong>HoH Winner:</strong><span>${esc(r.hoh)}</span></div>${r.wildcard?`<div><strong>Special Competition Winner:</strong><span>${esc(r.wildcard)}</span></div>`:""}<div><strong>Initial Nominees:</strong><span>${esc(r.initial)}</span></div><div><strong>PoV Winner:</strong><span>${esc(r.pov)}</span></div><div><strong>Veto Used:</strong><span>${esc(r.used)}</span></div><div><strong>Replacement:</strong><span>${esc(r.replacement)}</span></div><div><strong>Final Nominees:</strong><span>${esc(r.final)}</span></div><div><strong>Evicted:</strong><span>${esc(r.evicted)}</span></div></div></article>`).join("");
  ...
  function updateSeasonUI(){
    ...
    seasonHeading.textContent=state.season.name||"Big Brother 19";
